...
from main import optimize_single_hardware_co_with_tiling_optimization


def parse_args():
    parser = argparse.ArgumentParser(
        description="Run depfin2 optimization flow through main.optimize_single_hardware_co_with_tiling_optimization."
    )
    parser.add_argument(
        "--experiment-id",
        type=str,
        default="depfin2-main-ga-tiling-opt-test",
    ...
    parser.add_argument(
        ...
        choices=["ga", "ilp"],
        default="ga",
        help="Method used to optimize tile-group core allocation.",
    )
    parser.add_argument(
        "--max-inter-core-factor",
        type=int,
        default=16,
        help="Upper bound on inter-core tiling factor candidates.",
    )
    parser.add_argument(
        "--max-intra-core-factor",
        type=int,
        default=8,
        help="Upper bound on intra-core tiling factor candidates.",
    )
    parser.add_argument(
        "--tile-alloc-ga-generations",
        type=int,
        default=12,
        help="GA generations for tile-group-to-core allocation optimization.",
    )
    parser.add_argument(
        "--tile-alloc-ga-population",
        type=int,
        default=24,
        help="GA population size for tile-group-to-core allocation optimization.",
    )
    parser.add_argument(
        "--tile-alloc-random-seed",
        type=int,
        default=0,
        help="Random seed for tile allocation optimization.",
    )
...
def create_soc_template(template_path: Path) -> None:
    template_content = dedent(
        """
        name: {{ soc_name | default('depfin2_main_tiling_opt_soc', true) }}
        cores:
          0: ./cores/core.yaml
...
def main():
    ...
    hw_template_path = run_dir / "soc_template_for_main_tiling_opt.yaml"
    create_soc_template(hw_template_path)

    template_params = {
        "soc_name": "depfin2_main_tiling_opt_soc",
        "bus_bw": 64,
    }
    ...
    logging.info("Starting optimize_single_hardware_co_with_tiling_optimization flow")
    scme = optimize_single_hardware_co_with_tiling_optimization(
        ...
        tiling_optimization_method=args.tiling_optimization_method,
        max_inter_core_factor=args.max_inter_core_factor,
        max_intra_core_factor=args.max_intra_core_factor,
        tile_alloc_ga_generations=args.tile_alloc_ga_generations,
        tile_alloc_ga_population=args.tile_alloc_ga_population,
        tile_alloc_random_seed=args.tile_alloc_random_seed,
    )
    ...
    summary_path = run_dir / "main_ga_tiling_opt_summary.json"
