...
from stream.workload.computation.computation_node import (
    LOOP_RANGES_T,
    ComputationNode,
    GeneratedComputationNode,
)
...
def deduce_tensor_reuse_factors(
    ...
    """
    constant_operands = (
        original_node.constant_operands + original_node.partially_constant_operands
    )
...
class TiledWorkloadGenerationStage2(Stage):
    ...
    def get_all_node_pairs(
        g: ONNXWorkload,
    ) -> tuple[tuple[ComputationNode, ComputationNode, bool], ...]:
    ...
    def get_outer_tmap_loop_dimensions(
    ...
    def get_total_outer_size(
        self, outer_temporal_loops: list[TemporalLoop], dim: LayerDim
    ):
        """Return the total outer temporal size for the given dim."""
        return prod(
            [loop.size for loop in outer_temporal_loops if loop.dimension == dim]
        )
    ...
    def get_tiles(
        ...
        tile_attrs = self._pad_layer_dim_sizes(
            tile_attrs, outer_temporal_loops, mandatory_divisors
        )
        ...
        tile_attrs = self._take_away_outer_temporal_loops(
            tile_attrs, outer_temporal_loops
        )
        ...
        for n in range(nb_cns):
            ...
            produces_final_output = self.produces_final_output(
                original_node, dim_min_max
            )
            ...
            for constant_operand in (
                tile.constant_operands + tile.partially_constant_operands
            ):
    ...
    def _pad_layer_dim_sizes(
        ...
        for dim, size in tile_attrs.layer_dim_sizes.items():
            new_size = self.pad_until_divisible(
                dim, int(size), outer_temporal_loops, mandatory_divisors
            )
            if size != new_size:
                tile_attrs.layer_dim_sizes[dim] = new_size
                logger.warning(
                    f"Padded layer dimension {dim}: {size} -> {new_size} to be divisible by tiling factors"
                )
        return tile_attrs
    ...
    def _get_dim_min_max(
        ...
        outer_loop_values = self._get_outer_loop_values(
            n, outer_temporal_loops, stop_values
        )
    ...
    def produces_final_output(
        self, node: ComputationNode, dim_min_max: LOOP_RANGES_T
    ) -> bool:
        """If all the output irrelevant loops are at a max, this is producing a final output, so set a flag"""
        original_node_output_ir_dims = node.loop_relevancy_info.get_ir_layer_dims(
            Constants.OUTPUT_LAYER_OP
        )
        return all(
            [
                dim_min_max[dim][1] >= node.layer_dim_sizes[dim]
                for dim in original_node_output_ir_dims
            ]
        )

    def _replace_identical_tensors(
        self, tile: ComputationNode, previous_tensors: list[Tensor]
    ):
    ...
    def _set_core_allocation_for_tile(
        self, tile: ComputationNode, group_id: int, original_node: ComputationNode
    ):
    ...
    def build_rtree(
        ...
        rtree = index.Index(
            self.bounding_box_generator(producer, consumer, nodes, operand),
            properties=props,
        )
        return rtree
    ...
    def flatten_grouped_convolution_ranges(
        ...
        if dim_G in dims_copy and (dim_C in dims_copy or dim_K in dims_copy):
            ...
            CH_max_incl = (
                G_max_incl * original_node.layer_dim_sizes[dims_copy[C_K_idx]]
                + C_K_max_incl
            )
    ...
    def get_inter_edges_rtree(
        ...
        for input_operand in dependent_input_operands:
            ...
            consumer_tree = self.build_rtree(
                producer, consumer, consumer_tiles, input_operand
            )
            ...
            for producer_tile in producer_tiles:
                ...
                ir_dims_not_at_max = [
                    producer_tile.loop_ranges[ir_dim][1]
                ...
                for intersecting_consumer_node_id in intersecting_consumer_node_ids:
                    intersecting_consumer = consumer_tiles[
    ...
    def get_inter_edges_numpy(
        ...
        all_inter_edges: list[
            tuple[ComputationNode, ComputationNode, dict[str, Any]]
        ] = []
        paths_between = self.workload.find_paths_with_intermediate_type(
            producer, consumer, PropagationNode
        )
        ...
        for path_between in paths_between:
            ...
            assert isinstance(first_node, ComputationNode), (
                "First node in path should be ComputationNode"
            )
            tensor = self.get_tensor_cn_for_op(
                first_node, dependent_operand=Constants.OUTPUT_LAYER_OP
            )
            timesteps += (time.time(),)
            ...
            for i, node in enumerate(path_between[1:-1], start=1):
                assert isinstance(node, PropagationNode), (
            ...
            assert isinstance(final_node, ComputationNode), (
                "Last node in path should be ComputationNode"
            )
            dependent_operand = self._get_dependent_operand(node, final_node)
            inter_edges = self.get_inter_edges_hybrid(
                tensor, final_node, dependent_operand, relevant_axes
            )
    ...
    def _print_time_delta_to_logger(
        self, timesteps: tuple[float, float, float], path: str
    ):
    ...
    def _get_dependent_operand(self, producer: Node, consumer: ComputationNode):
        ...
        return next(
            op
            for op, dependent_node_id in consumer.input_operand_source.items()
            if dependent_node_id == producer.id
        )
    ...
    def _initialize_relevant_axes(self, node: ComputationNode, tensor: NodeTensor):
        ...
        for dim, size in tilings:
            assert isinstance(size, int), (
                f"Size of tiling {dim} should be an integer, got {size}"
            )
    ...
    def get_tensor_cn_for_op(
        self, node: ComputationNode, dependent_operand: LayerOperand
    ):
    ...
    def get_inter_edges_hybrid(
        ...
        for consumer_tile in self.tiles_dict[final_node]:
            ...
            slices = tuple(
                slice(start, stop) for start, stop in relevant_loop_ranges
            ) + (Ellipsis,)
    ...
    def get_inter_edges_tensor_based(
        producer_output_tensor: NodeTensor, consumer_input_tensor: NodeTensor
    ):
        ...
        if producer_output_tensor.tensor_shape != consumer_input_tensor.tensor_shape:
            raise TensorDimensionMismatchError(
    ...
    def get_node_tensor(
        ...
        assert node.pr_layer_dim_sizes is not None, (
            f"Node {node} must have pr_layer_dim_sizes set"
        )
        all_loop_dim_sizes = node.layer_dim_sizes + node.pr_layer_dim_sizes  # union
        tensor_shapes: tuple[int, ...] = tuple(
            all_loop_dim_sizes[dim] for dim in tensor_dims
        )
        ...
        if op == node.output_operand:
            ...
            should_add_to_tensor_list = [
                all(
                    tile.loop_ranges[ir_dim][1] >= node.loop_ranges[ir_dim][1]
        ...
        for tile, should_add_to_tensor in zip(
            tile_list, should_add_to_tensor_list, strict=False
        ):
            ...
            bounded_op_dim_ranges = tuple(
                ...
                for ((start, stop), max_stop) in zip(
                    op_dim_ranges, op_dim_ranges_max_stop, strict=False
                )
            )
    ...
    def get_node_tensors(
        self, node: ComputationNode, tiles: list[ComputationNode]
    ) -> dict[LayerOperand, NodeTensor]:
        variable_operands = [
            op for op in node.input_operands if op not in node.constant_operands
        ] + [node.output_operand]
    ...
    def set_nb_real_predecessors(workload: ComputationNodeWorkload):
        ...
        for node in workload.node_list:
            real_predecessors = [
                pred for pred in workload.predecessors(node) if pred.id != node.id
            ]
            node.nb_real_predecessors = len(real_predecessors)

    def remake_workload(
        self, tiles: list[ComputationNode], edges: list[EDGE_T]
    ) -> ComputationNodeWorkload:
